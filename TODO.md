...
`base.html` now carries a real `<title>`, a `meta description`, a `canonical`, Open Graph
and a Twitter card; `threat.html` carries the page's one `<h1>`; and `/robots.txt` exists.
The title and the description are built per scan, so a scan URL names the pilots it
profiles instead of every URL being titled `lscan`.

**The call that was made: scan URLs are indexable.** The pilot names are in the path now,
so they are the long tail, and `robots.txt` is `Allow: /` with only `/healthz` disallowed.
Two consequences to keep in view:

- **Third-party player names are published on our domain**, assembled from somebody
  else's paste. That was the accepted trade for the traffic.
- **Crawlers can walk into the expensive path.** `Crawl-delay: 10` is the only brake and
  it is a weak one - Bing and Yandex honour it, Google ignores it. The real guard is the
  `MAX_SCAN_ROWS` pre-flight count, which refuses a scan rather than truncating it. Watch
  this once the 365-day backfill lands: the same paste that costs 2.0-2.7 s today is
  projected at ~30 s then, and a crawler does not get bored.

Second pass, done:

- **The canonical uses the spelling ESI returns.** ESI resolves a name case-insensitively,
  so `/jaja_colene`, `/JAJA_COLENE` and `/Jaja_Colene` were three live URLs each claiming
  to be the original. They now all carry one canonical. The page also re-spells the scan in
  `data-names`, so the first window or filter click rewrites the address bar to the
  canonical form. A name ESI cannot resolve keeps the visitor's spelling - the page is
  reporting that typo back to them.
- **Fragment responses carry `X-Robots-Tag: noindex`.** `?fragment=blocks|detail|targets`
  returns a bare row list with no `<head>`. Nothing links to one, but the URLs are
  guessable and an indexed fragment is a junk result.
- **A trailing slash redirects instead of 404ing** (`/x/` -> `/x`), and `/favicon.ico` at
  the root redirects to the static file. `APPEND_SLASH` only ever adds a slash, so both
  needed explicit routes.
- **A full favicon set**, rasterised from `favicon.svg` with the headless Chromium. Note
  the 16px file is a *tighter crop* of the same mark on purpose: the three radar arcs turn
  to mush below about 24px, so it shows fewer of them. Everything from 32px up is the full
  glyph. Regenerate all of them together if the mark ever changes.

**Deliberately not done: collapsing the window variants.** `/x`, `/x/30`, `/x/180` and
`/x/365` still each canonicalise to themselves, so one pilot list is four indexable pages
with different numbers. Pointing them all at the default-window URL is a one-line change if
the thin-variant duplication ever shows up in Search Console.

Still open here:

- **The `og:image` is a real screenshot of the UI with fabricated numbers.** It profiles
  `Moe Sten` (the owner's own character) with invented figures. The killmails were
  generated, then passed through the real `build_profile` and the real
  `_char_blocks.html`, so the card cannot show a layout the app does not produce. Rebuild
  it whenever the row layout changes. The numbers are a product shot, not a claim, and
  they do not match zKillboard.
- **There is still nothing to rank for beyond the tool itself.** The landing page is a
  paste box plus one heading. Ranking for "eve local scan" means writing content - an
  explainer of what the metrics mean, maybe per-metric pages - which is a product
  decision, not a header change.
- **No `sitemap.xml`.** A sitemap of one URL buys nothing, and scan URLs are generated by
  visitors rather than by us, so there is no list to publish.
- **No structured data.** If `<script type="application/ld+json">` is ever added, check it
  against the planned CSP first (`script-src 'self'` would block an inline block).

Done alongside, and worth knowing:

- **The portraits are lazy now.** `_char_blocks.html` requests one 64px portrait plus
  corp/alliance logos per pilot from `images.evetech.net` - ~180 unsized third-party images
  on a 60-pilot scan - and all three now carry `loading="lazy" decoding="async"`. Note the
  earlier claim in this file that they were the CLS source **was wrong**: `intel.css`
  already fixes `.cs-portrait` at 48px and `.cs-aff img` at 18px, and the stylesheet is
  same-origin and render-blocking, so the boxes were always reserved. The cost was the
  eager fan-out, not layout shift.
- **URL length is bounded twice.** `NAMES_RE` caps the names segment at 4000 characters to
  match gunicorn's `--limit-request-line 4094`; `MAX_CHARACTERS` still caps the pilot count
  and reports the overflow. A list long enough to blow the request line was never servable,
  under the old query-string scheme either.
